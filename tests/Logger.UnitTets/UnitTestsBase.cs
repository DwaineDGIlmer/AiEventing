...
namespace Logger.UnitTets
{
    ...
    class MockFaultAnalysis(string content, string role) : IFaultAnalysisService
    {
        ...
        /// cref="OpenAiChatResponse"/>  object, which includes the generated response and associated metadata.</returns>
        public Task<OpenAiChatResponse> AnalyzeFaultAsync(IList<OpenAiMessage> messages)
        {
            return Task.FromResult(new OpenAiChatResponse
            {
                Choices = [ new CompletionChoice()
            {
        ...
        public Task<bool> AnalyzeFaultAsync(ILogEvent fault)
        {
